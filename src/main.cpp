...
#if defined(NDEBUG)
#error "Bederl cannot be compiled without assertions."
#endif
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from Bederl
...
{
    RenameThread("bederl-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZBEDMinted()
{
...
void RecalculateZBEDSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zBED supply
...
bool RecalculateBEDSupply(int nHeightStart)
{
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert BED to zBED
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // Bederl: We use certain sporks during IBD, so check to see if they are
